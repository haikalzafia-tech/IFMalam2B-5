@extends('layouts.kai')
@section('page_title', $pageTitle)
@section('content')
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <div>
            <h4 class="card-title mb-0">#{{ $transaksi->nomor_transaksi }}</h4>
        </div>
        <a href="{{ route('transaksi-keluar.index') }}" class="btn btn-primary btn-round btn-sm">
            <i class="fas fa-arrow-left"></i> Kembali
        </a>
    </div>
    <div class="card-body">
        <x-meta-item label="penerima" value="{{ $transaksi->penerima }}" />
        <x-meta-item label="Kontak" value="{{ $transaksi->kontak }}" />
        <x-meta-item label="Jumlah Barang" value="{{ number_format($transaksi->jumlah_barang)}} pcs" />
        <x-meta-item label="Total Harga" value="Rp. {{ number_format($transaksi->total_harga)}}" />
        <x-meta-item label="Keterangan" value="{{ $transaksi->keterangan }}" />
        <x-meta-item label="Petugas" value="{{ $transaksi->petugas }}" />
        <x-meta-item label="Tanggal" value="{{ $transaksi->formated_date }}" />

        <div class="mt-4">
            <h6>Detail Barang</h6>
            <table class="table">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nomor Barang</th>
                        <th>Jumlah</th>
                        <th>Harga Satuan</th>
                        <th>Total Harga</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($transaksi->items as $index => $item)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $item->produk }} {{ $item->nama_varian }}</td>
                        <td>{{ $item->qty }}</td>
                        <td>Rp. {{ number_format($item->harga) }}</td>
                        <td>Rp. {{ number_format($item->subTotal) }}</td>
                    </tr>
                    @endforeach
                </tbody>
                <tfoot>
                    <tr>
                        <th colspan="4">Grand Total</th>
                        <th>Rp. {{ number_format($transaksi->total_harga) }}</th>
                    </tr>
                </tfoot>
            </table>
        </div>
    </div>
</div>
@endsection
